...
Software engineering subject last-term scheme
...
- Software features:
  ...
  + Storage:
    ...
    + Books are stored in database with its code, name, barcode, deal, etc…
...
+ Save all invoice after transaction easily in online database and local databse.
    ...
    + Regular customers also get more discount than other one because we have their information in our database.
   + Về mặt bán hàng:
   + Selling:
     ...
     + Export invoice for each transaction (include serial, name of product, salesman, customer’s information,…). If customer does not satisfied, they can easily return (read Product Warranties and returns below).
     ...
     + Any promotion, discount of selected books will be given in customer’s invoice.
   ...
   + Product Warranties and returns:
     ...
     + Delete unused invoice (out of warranty, more than 2 years) to give more free space on hard disk and make it easily to search products.
     
   + Ngoài ra ứng dụng còn có khả năng phân quyền truy cập giúp đảm bảo được tính bảo mật cho khách hàng sử dụng.
  + Only authorized person have access to use this software to protect information.
...
- We will keep working on update to introduce new helpful features such as preview books on this software, so that customers can easily see what they will get. 

- Application:
  + Name: Bookstore Management App
  + Version: 1.0.0
  + This software can be used as an information management for bookstore, stationery shop, etc…
